...
HRESULT InitDevice()
{
   ...
   // g_deviceContext.m_deviceContext->OMSetRenderTargets( 1, &g_pRenderTargetView, g_pDepthStencilView );
    ...
    hr = g_shaderProgram.init(g_device, "Inosuke_Engine.fx", Layout);
    if (FAILED(hr)) {
      ERROR("Main", "InitDevice",
    ...
    //Integrar los vertices a mershcomponent 
    for (unsigned int i = 0; i < 24; i++) {
      g_mesh.m_vertex.push_back(vertices[i]);
    }
    g_mesh.m_numVertex = 24;
    
    //Integrar los indices a mershcomponent 
    ...
    // Set index buffer
    //g_deviceContext.m_deviceContext->IASetIndexBuffer( g_pIndexBuffer, DXGI_FORMAT_R32_UINT, 0 );
